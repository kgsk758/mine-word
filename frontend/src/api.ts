...
export const searchVector = async (vector: number[], k: number = 5): Promise<{ word: string; score: number }[]> => {
  const response = await axios.post(`${API_BASE_URL}/api/search`, { vector, k });
